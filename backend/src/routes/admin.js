...
router.get('/planilha-geral', async (req, res, next) => {
  ...
  try {
    const [{ rows: empresas }, { rows: usuarios }] = await Promise.all([
      pool.query('SELECT id, nome, tipo_operacao, criado_em FROM empresas ORDER BY criado_em'),
      pool.query(`
        SELECT u.nome, u.email, u.tipo, u.criado_em, e.nome AS empresa_nome
    ...
    const wsEmpData = [
      ...
      ['', '', '', ''],
      ['Nome', 'Tipo de Operação', 'Cadastrada em', 'Total Usuários'],
      ...empresas.map(e => [
        e.nome,
    ...
    wsEmp['!cols'] = [{ wch: 24 }, { wch: 24 }, { wch: 16 }, { wch: 14 }];
    ...
    const wsUsrData = [
      ...
      [`Gerado em: ${new Date().toLocaleString('pt-BR')}`, '', '', '', ''],
      ['', '', '', '', ''],
      ['Empresa', 'Nome', 'Email', 'Tipo', 'Cadastrado em'],
      ...usuarios.map(u => [
        u.empresa_nome,
        u.nome,
        u.email,
    ...
    wsUsr['!cols'] = [{ wch: 24 }, { wch: 24 }, { wch: 34 }, { wch: 16 }, { wch: 16 }];
