...
export default function ImportacaoCandidatos() {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      const { data } = await api.post('/importar-candidatos/preview', fd);
      setPreview(data);
  ...
  async function confirmar() {
    setLoading(true);
    try {
      const { data } = await api.post('/importar-candidatos/confirmar', { arquivo });
      setResultado(data);
      setStep('done');
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Importar Candidatos</h1>
        <p className="text-sm text-gray-500 mt-0.5">Faça upload de uma planilha para importar candidatos em massa</p>
      </div>
        ...
        {['upload', 'preview', 'done'].map((s, i) => (
          <div key={s} className="flex items-center gap-1 sm:gap-2 shrink-0">
            <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
              ...
              : (['preview', 'done'].includes(step) && i === 0) || (step === 'done' && i === 1)
      ...
      {step === 'upload' && (
        <div className="space-y-4">
          ...
          <div className="card bg-green-50 border-green-100">
            <h3 className="font-medium text-green-900 mb-2 text-sm">Colunas reconhecidas automaticamente:</h3>
            <div className="flex flex-wrap gap-2">
              {['Nome', 'Telefone / Celular / WhatsApp', 'Email', 'Cargo / Vaga / Função', 'Canal / Origem (whatsapp ou linkedin)'].map(c => (
                <span key={c} className="bg-white text-green-700 text-xs px-2 py-1 rounded-lg border border-green-200">{c}</span>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-4">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    {['Nome', 'Telefone', 'Email', 'Cargo', 'Canal'].map(h => (
                ...
                <tbody className="divide-y divide-gray-50">
                  {preview.preview.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-gray-500">{row.cargo || '—'}</td>
                      <td className="px-4 py-2 text-gray-500">{row.canal || 'whatsapp'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button onClick={confirmar} className="btn-primary flex items-center justify-center gap-2" disabled={loading}>
              {loading ? 'Importando...' : <><Upload size={16} /> Confirmar ({preview.total} candidatos)</>}
            </button>
      ...
      {step === 'done' && resultado && (
        <div className="card text-center space-y-4">
          ...
          <div className="flex justify-center gap-3 pt-2">
            ...
            <a href="/recrutamento/candidatos" className="btn-primary">Ver candidatos</a>
          </div>
        </div>
      )}
    </div>
